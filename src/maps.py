def risk_map(risk):
    if risk in ["noise", "outliers", "robustness", "noisy labels", "label noise", "input perturbation", "input corruption", "anomaly detection", "unseen class", "metric robustness", "out-of-distribution (ood)"]:
        return "Noise and\noutliers"
    
    elif risk in ["generalization", "non-stationarity", "stability-plasticity", "uncertainty", "partial information", "domain adaptation", "spurious correlation", "domain generalization", "data sampling", "transfer learning"]:
        return "Non-stationary\ndistribution"
    
    elif "adversarial" in risk or risk in ["misuse", "model poisoning", "backdoor injection attack", "synthetic data poisoning", "label poisoning"]:
        return "Adversarial\nattack"
    
    elif risk in ["unsafe actions", "unsafe exploration", "unsafe states", "exploration", "constraint violation"]:
        return "Unsafe\nexploration"
    ...
    elif risk in ["unintended behaviour", "emergent behaviour", "hallucination", "reward signal", "deceptive agent", "rogue agent", "reward signal corruption", "transition function corruption", "wireheading", "untruthfulness", "selfish agent", "self-modification"]:
        return "Undesirable\nbehaviour"
    
    elif risk in ["malware", "phishing", "data sharing", "watermarking"]:
        return "Lack of control\nenforcement"
    
    elif risk in ["heterogeneous data", "data quality", "multi-modal data", "limited data", "missing data", "data poisoning", "high-dimensional control", "scarce resource allocation"]:
        return "System\nmisspecification"
    ...
    elif risk in ["model requirements", "problem requirements", "data requirements", "resource constraints", "modeling errors", "domain definition", "capacity limit", "safety requirements", "data efficiency", "system specification", "systemic safety", "hyper-parameter tuning", "communication bottleneck", "standardization"]:
        return "System\nmisspecification"
